...
class HostInfoCommand extends Command {
    protected static $defaultName = 'cron:host-info';
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($virtType != 'kvm' || (isset($pool) ? $pool : null) != 'zfs') {
        ...
        if (!isset($out) && file_exists('/usr/sbin/vzctl')) {
            ...
            if ($out != '') {
                ...
                $usedg = ceil($usedb / 1073741824);
                $out = $totalg.' '.$freeg;
            } elseif ($virtType != 'kvm' || (isset($pool) ? $pool : null) != 'zfs') {
        ...
        if ($dispJson) {
            $output->writeln(json_encode($server, JSON_PRETTY_PRINT));
        }
        ...
        $output->writeln(trim(shell_exec($cmd)));
        return 0;
    }
}
