...
class UpdateCommand extends Command
{
    ...
    {
        $this
            ->setDescription("Updates a Virtual Machine setting HD, Ram, CPU, Cgroups.")
            ->addArgument('vzid', InputArgument::REQUIRED, 'VPS id/name to use')
            // options (matching original CLIFramework usage)
            ...
            ->addOption('hd', 'h', InputOption::VALUE_REQUIRED, 'HD Size in GB')
            ->addOption('ram', 'r', InputOption::VALUE_REQUIRED, 'Ram Size in MB')
            ->addOption('cpu', 'c', InputOption::VALUE_REQUIRED, 'Number of CPU/Cores')
            ->addOption('cgroups', 'g', InputOption::VALUE_REQUIRED, 'Update CGroups to number of slices')
            ->addOption('timezone', 'z', InputOption::VALUE_REQUIRED, 'changes the timezone')
            ->addOption('hostname', 'n', InputOption::VALUE_REQUIRED, 'changes the hostname')
            ->addOption('password', 'p', InputOption::VALUE_REQUIRED, 'Sets the root/Administrator password')
            ->addOption('password-reset', null, InputOption::VALUE_NONE, 'Sets the root/Administrator password')
            ->addOption('username', 'u', InputOption::VALUE_REQUIRED, 'Sets the password for the given username instead of the root/Administrator')
            ->addOption('quota', 'q', InputOption::VALUE_REQUIRED, 'Enable or Disable Quotas setting them to on or off')
            ->addOption('io-limit', null, InputOption::VALUE_REQUIRED, 'The IO Limit in bytes/s')
            ->addOption('iops-limit', null, InputOption::VALUE_REQUIRED, 'The IO Limit in iops')
        ;
    }
     ...
     * Execute the command.
     *
     * This conversion preserves the original logic and relies on Vps::getLogger()
     * for output as in the original file.
     */
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        //
        // Build an $opts-like structure similar to the CLIFramework behavior used
        // throughout the original command. The original code expected $this->getOptions()
        // and used $opts->keys with array_key_exists() checks; we create an object
        // with a 'keys' associative array where presence indicates the option was passed.
        //
        $opts = new \stdClass();
        $opts->keys = [];

        // List of option names we want to inspect/preserve
        $optionNames = [
            'verbose','virt','hd','ram','cpu','cgroups','timezone','hostname',
            'password','password-reset','username','quota','io-limit','iops-limit'
        ];

        foreach ($optionNames as $name) {
            // For flags use boolean existence; for value options, only include when non-null
            $val = $input->getOption($name);
            // Symfony returns null when option wasn't provided (for VALUE_REQUIRED),
            // and boolean true/false for VALUE_NONE flags.
            if ($val !== null && $val !== false) {
                $opts->keys[$name] = (object)['value' => $val];
            }
        }

        // Initialize Vps with the options object to mimic original behavior.
        // The original call was: Vps::init($this->getOptions(), ['vzid' => $vzid]);
        Vps::init($opts, ['vzid' => $vzid]);

        // Shortcuts to keep original variable names/logic
        $base = Vps::$base;
        ...
        // If any of these updates require stopping the VPS, stop it.
        ...
        $isRunning = Vps::isVpsRunning($vzid);
        ...
        // -----------------------------
        // Prepare XML for kvm when changing CPU/RAM/timezone
        // -----------------------------
        ...
        // -----------------------------
        // Finalize kvm xml changes (if any)
        // -----------------------------
        if ($updateCpu === true || $updateRam === true || $updateTimezone === true) {
            if (Vps::getVirtType() == 'kvm') {
        ...
        // -----------------------------
        // IO / IOPS limits for different virt types
        // -----------------------------
        if (Vps::getVirtType() == 'kvm') {
            if ($updateIoLimit !== false) {
                Vps::getLogger()->write(Vps::runCommand("virsh blkdeviotune {$vzid} vda --total-bytes-sec ".$opts->keys['io-limit']->value." --config"));
                if ($isRunning === true) {
                    Vps::getLogger()->write(Vps::runCommand("virsh blkdeviotune {$vzid} vda --total-bytes-sec ".$opts->keys['io-limit']->value." --live"));
                }
            }
            if ($updateIopsLimit !== false) {
                Vps::getLogger()->write(Vps::runCommand("virsh blkdeviotune {$vzid} vda --total-iops-sec ".$opts->keys['iops-limit']->value." --config"));
                if ($isRunning === true) {
                    Vps::getLogger()->write(Vps::runCommand("virsh blkdeviotune {$vzid} vda --total-iops-sec ".$opts->keys['iops-limit']->value." --live"));
                }
            }
        } elseif (Vps::getVirtType() == 'virtuozzo') {
            if ($updateIoLimit !== false)
        ...
        // -----------------------------
        // Restart VPS if we stopped it earlier for changes
        // -----------------------------
        if ($updateCpu === true || $updateRam === true || $updateHd === true || $updateTimezone === true || $updateHostname === true || $updatePassword === true || $updatePasswordReset === true) {
            Vps::startVps($vzid);
        }
