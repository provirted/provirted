...
class BwInfoCommand extends Command {
    protected static $defaultName = 'cron:bw-info';
    ...
    public function configure()
    {
        ...
        $this->addOption('json', 'j', InputOption::VALUE_NONE, 'Display data in JSON format');
        $this->addOption('all', 'a', InputOption::VALUE_NONE, 'Use All Available HD, CPU Cores, and 70% RAM');
    }

    protected function execute(InputInterface $input, OutputInterface $output)
    {
        // Build an options array similar to the original getOptions() object
        $optsArray = [
            'verbose' => $input->getOption('verbose') ? 1 : 0,
            'virt'    => $input->getOption('virt'),
            'json'    => $input->getOption('json') ? 1 : 0,
            'all'     => $input->getOption('all') ? 1 : 0,
        ];

        Vps::init($optsArray, []);

        $useAll = $optsArray['all'] === 1;
        $dispJson = $optsArray['json'] === 1;
        ...
        if ($dispJson) {
            ...
            $output->writeln("vpsName2Veid: ".json_encode($vpsName2Veid, JSON_PRETTY_PRINT));
            $output->writeln("vpsVeid2Name: ".json_encode($vpsVeid2Name, JSON_PRETTY_PRINT));
            $output->writeln("IPs: ");
            $output->writeln(json_encode($ips, JSON_PRETTY_PRINT));
            $output->writeln("Totals: ");
            $output->writeln(json_encode($totals, JSON_PRETTY_PRINT));
        }
