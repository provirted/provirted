...
class RestoreCommand extends Command {
	...
	public function execute($source, $name, $vzid, $id) {
		...
		$base = Vps::$base;
		Vps::getLogger()->write(Vps::runCommand("{$base}/vps_swift_restore.sh {$source} {$name} {$vzid} && curl --connect-timeout 60 --max-time 600 -k -d action=restore_status -d vps_id={$id} https://mynew.interserver.net/vps_queue.php || curl --connect-timeout 60 --max-time 600 -k -d action=restore_status -d vps_id={$id} https://mynew.interserver.net/vps_queue.php"));
	}
}
